{- |
Module      :  XMonad.Prompt.FuzzyShell

A shell prompt with fuzzy matching for XMonad
-}
...
data FuzzyShell = FuzzyShell

instance XPrompt FuzzyShell where
    showXPrompt FuzzyShell     = "Run: "
    completionToCommand _ = escape

fuzzyShellPrompt :: XPConfig -> X ()
fuzzyShellPrompt c = do
    cmds <- io getCommands
    mkXPrompt FuzzyShell c (getShellCompl cmds) spawn
...
prompt = unsafePrompt
safePrompt c config = mkXPrompt FuzzyShell config (getShellCompl [c]) run
    where run = safeSpawn c . return
unsafePrompt c config = mkXPrompt FuzzyShell config (getShellCompl [c]) run
...
getShellCompl cmds s | s == "" || last s == ' ' = return []
    ...
    files <- case f of
               ...
               _   -> return f
    return . uniqSort $ files ++ fuzzyCompletionFunction cmds s

fuzzyMatch :: String -> String -> Bool
fuzzyMatch [] test = True
fuzzyMatch (x:xs) test = (elem x test) && (fuzzyMatch xs $ tail $ dropWhile (/= x) test)

fuzzyCompletionFunction :: [String] -> String -> [String]
fuzzyCompletionFunction cmds str | '/' `elem` str = []
                                 | otherwise      = filter (fuzzyMatch str) cmds
