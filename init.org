...
**** Org clock daytime
See [[https://github.com/ArneBab/emacs-org-clock-daytime][emacs-org-clock-daytime]] repository.
#+begin_src emacs-lisp
  ;; (use-package emacs-org-clock-daytime
  ;;   :straight (:host github :repo "ArneBab/emacs-org-clock-daytime"))
#+end_src
...
Manually set plasmashell as an unmanaged window, otherwise EXWM thinks it is a regular window.
#+begin_src emacs-lisp
  (setq exwm-manage-configurations
        '(((string= exwm-instance-name "plasmashell") managed nil)))
#+end_src
...
** Beginend
Go to the "logical beginning" of the file instead of the actual beginning. See the code [[https://github.com/DamienCassou/beginend][here]].
#+begin_src emacs-lisp :results silent
  (use-package beginend :straight t :config (beginend-global-mode))
#+end_src
...
#+begin_src emacs-lisp
  (use-package citar
    ...
    (citar-notes-paths '("~/Org/Roam/Bibnotes/"))
  ...
  (use-package citar-org-roam
    ...
    (citar-org-roam-subdir "Bibnotes/")
...
*** Ob-penrose
Source code [[https://github.com/weavermarquez/ob-penrose][here]].
#+begin_src emacs-lisp
    (use-package ob-penrose :straight (:host github :repo "weavermarquez/ob-penrose")
      :after org :defer t)
#+end_src
...
#+begin_src emacs-lisp
  ...
  ;; If you want to complete snippets using completion-preview, corfu-mode or company-mode
  ;;(add-to-list 'completion-at-point-functions #'yankpad-capf)
...
#+begin_src emacs-lisp
  (setq mu4e-bookmarks
        '((:name  "Reasonable recent messages"
          ...
          (:name "Queued messages" :query "(maildir:/queued-mail)" :key ?q)))
#+end_src
...
#+begin_src emacs-lisp
  (setq mu4e-maildir-shortcuts
        '((:maildir "/ANU/INBOX" :key ?a)
          ...
          (:maildir "/MIT" :key ?m)
          (:maildir "/UChicago" :key ?c)))
#+end_src

*** Attachments
Save all attachments without any prompts. The attachments are saved to the default attachment directory, which has been set to "/tmp/".
The code is based on the snippets found on https://macowners.club/posts/update-mu4e-save-attachments-faster/.
#+begin_src emacs-lisp :results silent
  (defun ab/mu4e-save-all-attachments ()
    "Save all attachments from the current mu4e view buffer to the default attachment directory (usually \"/tmp\").
  Currently this applies to all mime parts that are \"attachment-like\". That is, those that have a filename associated to them."
    (interactive)
    (let* ((parts (mu4e-view-mime-parts))
           (candidates (seq-map (lambda (fpart) (cons (plist-get fpart :filename) fpart))
                                (seq-filter (lambda (part) (plist-get part :attachment-like)) parts)))
      ...
      (seq-map (lambda (fname)
                (let* ((part (cdr (assoc fname candidates)))
                       (path (mu4e--uniquify-file-name
                              (mu4e-join-paths default-directory (plist-get part :filename)))))
                  (mm-save-part-to-file (plist-get part :handle) path)
  		path))
               files)))
#+end_src
Attach the file in ~fpath~ to a selected headline from a selected or created org-roam node.
#+begin_src emacs-lisp :results silent
  (defun ab/org-attach-to-roam-node (fpath)
    (interactive)
    (progn (find-file fpath)
           (org-roam-node-find)
           (ignore-errors (consult-org-heading))
           (org-attach-attach fpath nil 'mv)))
#+end_src

#+begin_src emacs-lisp
  (defun ab/mu4e-org-attach-attachments ()
    (interactive)
    (let ((fpaths (ab/mu4e-save-all-attachments)))
      (save-window-excursion
        (seq-do #'ab/org-attach-to-roam-node fpaths))
...
*** Queueing mail
#+begin_src emacs-lisp
  (setq smtpmail-queue-dir "~/Mail/queued-mail/cur")
#+end_src
*** Mail merge
#+begin_src emacs-lisp
  (defun ab/queue-message (to subject body &optional context)
    "Queue a new message with the given TO, SUBJECT, and BODY fields. Given a context, compose the message in the given context. Otherwise, default to the ANU context."
    (let ((smtpmail-queue-mail t)
          (mu4e-compose-context-policy nil))
      (mu4e-context-switch t (if context context "ANU"))
      (compose-mail to subject)
      (message-goto-body)
      (insert body)
      (message-send-and-exit)))

  (defun ab/mail-merge (subject body-file recepients &optional context)
    "Given a list of receipients, mail merge using the given subject and file containing the body text. The file containing the body text is expected to have one replacement string, which will be replaced by the name of the recepient. Each entry in the list of recepients is a list of the form (EMAIL NAME)."
    (let ((body (f-read-text body-file)))
      (mapc (lambda (lst)
              (ab/queue-message (car lst) subject (format body (cadr lst)) context))
            recepients)))
#+end_src
*** Endnotes
...
#+begin_src emacs-lisp :results silent
  (use-package consult-mu
    :straight (:host github :repo "armindarvish/consult-mu")
    :after (mu4e consult)
    :custom
    (consult-mu-mark-previewed-as-read t)
    (consult-mu-preview-key "M-c")
    :config
    (require 'consult-mu-embark)
    (require 'consult-mu-contacts "~/.emacs.d/straight/repos/consult-mu/extras/consult-mu-contacts.el")
    )
#+end_src
* Elfeed
